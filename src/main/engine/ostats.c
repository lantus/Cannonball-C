...
int8_t OStats_cur_stage;
uint32_t OStats_score;
uint16_t OStats_route_info;
uint16_t OStats_routes[0x8]; 
int16_t OStats_frame_counter;
...
void OStats_clear_stage_times()
{
    ...
    for (i = 0; i < 15; i++)
    {
        OStats_stage_counters[i] = 0;

        for (j = 0; j < 3; j++)
            OStats_stage_times[i][j] = 0;
    }

    // Choose correct lookup table if timing bugs fixed
    OStats_lap_ms = Config_engine.fix_timer ? LAP_MS_60 : LAP_MS_64;
}

void OStats_clear_route_info()
{
    OStats_route_info = 0;
    OStats_routes[0] = OStats_routes[1] = OStats_routes[2] = OStats_routes[3] = 
    OStats_routes[4] = OStats_routes[5] = OStats_routes[6] = OStats_routes[7] = 0;
}
...
{
    if (Outrun_game_state != GS_INGAME) return;

    OStats_inc_lap_timer();

    if (Outrun_cannonball_mode == OUTRUN_MODE_ORIGINAL || Outrun_cannonball_mode == OUTRUN_MODE_CONT)
    {
        // Each stage has a standard counter that just increments. Do this here.
        OStats_stage_counters[OStats_cur_stage]++;
        OHud_draw_lap_timer(0x11016C, OStats_stage_times[OStats_cur_stage], ms_value);
    }

    else if (Outrun_cannonball_mode == OUTRUN_MODE_TTRIAL)
    {
        OStats_stage_counters[Outrun_ttrial.current_lap]++;
        OHud_draw_stage_number(OHud_translate(30, 2 + Outrun_ttrial.current_lap, 0x110030), (Outrun_ttrial.current_lap + 1), HUD_GREY);
        OHud_draw_lap_timer(OHud_translate(32, 2 + Outrun_ttrial.current_lap, 0x110030), OStats_stage_times[OStats_cur_stage], ms_value);
    }
}
...
void OStats_inc_lap_timer()
{
    ...
    if (++OStats_stage_times[OStats_cur_stage][2] >= (Config_engine.fix_timer ? 0x3C : 0x40))
    {
        // Looped MS, so add a second
        OStats_stage_times[OStats_cur_stage][2] = 0;
        OStats_stage_times[OStats_cur_stage][1] = outils_bcd_add(OStats_stage_times[OStats_cur_stage][1], 1);

        // Loop seconds, so add a minute
        if (OStats_stage_times[OStats_cur_stage][1] == 0x60)
        {
            OStats_stage_times[OStats_cur_stage][1] = 0;
            OStats_stage_times[OStats_cur_stage][0] = outils_bcd_add(OStats_stage_times[OStats_cur_stage][0], 1);
        }
    }

    // Get MS Value
    ms_value = OStats_lap_ms[OStats_stage_times[OStats_cur_stage][2]];
}

// Source: 0xBE4E
void OStats_convert_speed_score(uint16_t speed)
{
    ...
    OStats_update_score(score);
}

// Update In-Game Score. Adds Value To Overall Score.
//
// Source: 0x7340
void OStats_update_score(uint32_t value)
{
    if (Outrun_cannonball_mode == OUTRUN_MODE_TTRIAL)
        return;

    OStats_score = outils_bcd_add(value, OStats_score);

    if (OStats_score > 0x99999999)
        OStats_score = 0x99999999;

    OHud_draw_score_ingame(OStats_score);
}
...
{
    if (OStats_extend_play_timer)
    {
        // End Extend Play: Clear Text From Screen
        if (--OStats_extend_play_timer <= 0)
        {
            OHud_blit_text1(TEXT1_EXTEND_CLEAR1);
            OHud_blit_text1(TEXT1_EXTEND_CLEAR2);
            OHud_blit_text1(TEXT1_LAPTIME_CLEAR1);
            OHud_blit_text1(TEXT1_LAPTIME_CLEAR2);
        }
        // Extend Play: Flash Text
        else
        {
            int16_t do_blit = ((OStats_extend_play_timer - 1) ^ OStats_extend_play_timer) & BIT_3;

            if (do_blit)
            {
                if (OStats_extend_play_timer & BIT_3)
                {
                    if (Outrun_cannonball_mode == OUTRUN_MODE_TTRIAL)
                        OHud_blit_text_new(15, 8, "BEST LAP!", HUD_PINK);
                    else
                    {
                        OHud_blit_text1(TEXT1_EXTEND1);
                        OHud_blit_text1(TEXT1_EXTEND2);
                    }
                }
                else
                {
    ...
    else if (Outrun_game_state == GS_INGAME && OInitEngine_checkpoint_marker)
    {
        ...
        if (!Outrun_freeze_timer)
        {
            if (Outrun_cannonball_mode == OUTRUN_MODE_ORIGINAL)
                OStats_time_counter = outils_bcd_add(OStats_time_counter, STATS_TIME[time_lookup]);
            else if (Outrun_cannonball_mode == OUTRUN_MODE_CONT)
                OStats_time_counter = outils_bcd_add(OStats_time_counter, 0x55);
        }
        ...
        OHud_draw_lap_timer(0x110554, OStats_stage_times[OStats_cur_stage-1], Config_engine.fix_bugs ? OStats_lap_ms[OStats_stage_times[OStats_cur_stage-1][2]] : ms_value);

        OTraffic_set_max_traffic();
        OSoundInt_queue_sound(sound_YM_CHECKPOINT);
        OSoundInt_queue_sound(sound_VOICE_CHECKPOINT);
        
        // Update Stage Number on HUD
        OHud_draw_stage_number(0x110d76, OStats_cur_stage+1, HUD_GREEN);
